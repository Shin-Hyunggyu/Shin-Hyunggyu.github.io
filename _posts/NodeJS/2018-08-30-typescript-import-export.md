...
ES6 에서 모듈을 불러오고 내보낼 수 있는 `import` 와 `export` 키워드가 새로 추가 되었다. 기존에는 환경에 따라 AMD, CommonJS, UMD 로 나누고 저 마다 각 모듈 시스템을 가지고 사용하고 있었다. 현재 CommonJS 기반의 NodeJS 에서는 부분적으로 ES6 사용이 가능한 것 같고 완벽한 ES6 또는 NEXT(최신버전) 를 사용하기 위해서는 [Babel](https://babeljs.io) 이나 [Typescript](https://www.typescriptlang.org) 의 도움을 받아야만 한다. 이번 포스팅은 Typescript 를 통해 `import` 와 `export` 를 사용했을 때 어떻게 transpile 되는지 알아 보려고 한다. 또 Babel 과도 어떤 차이가 있는지 비교해 볼 것이다. transpile target 은 가장 안정적으로 돌아가는 ES5 로 하겠다.
...
Typescript 에서 CommonJS 기반의 모듈을 어떻게 처리하는지 살펴보기 위해 '**3rd-module.js**' 를 추가했다. 아직까지는 `npm` 에 올라와 있는 대부분의 모듈들이 CommonJS 기반으로 제공되고 있다는 점을 유의해야 한다.
...
* 3rd-module.js (`npm install` 을 통해 다운받을 수 있는 CommonJS 기반의 모듈)
...
`module.exports` 와 `export default` 가 같다고 생각해 왔던 사람들에게는 세번째 값이 undefined 가 나온 것에 대해 의아해 할 수 있다. 기존에 사용하던 CommonJS 환경과 비슷하게 보였던 것은 여태껏 Typesciprt 가 transpile 되면서 `import` 와 `export` 를 사용하면 알아서 **default 프로퍼티**를 처리해 주었기 때문이다.
...
`exports` 변수에 프로퍼티를 추가하는 형태로 transpile 되었다. `export default` 가 `module.exports` 로 transpile 되지 않았다는 것에 유의해야 한다.
...
`import` 를 통해 모듈을 불러와 console 에서 사용할 때에도 **default 프로퍼티**를 추가하고 있다. 아마도 ES6 에서는 모듈을 내보낼 때 **default** 와 **property** 형태로 동시에 내보낼 수 있기 때문에 이렇게 처리하지 않았을까 생각해 본다. 따라서 console 에서 undefined 값이 나온 이유는 CommonJS 기반의 모듈이 **default 프로퍼티**가 없는 상태로 `export` 되었기 때문이다.

앞으로 `import` 를 **default 프로퍼티**가 있는 모듈을 불러오는 키워드라고 생각하면 되겠다.
