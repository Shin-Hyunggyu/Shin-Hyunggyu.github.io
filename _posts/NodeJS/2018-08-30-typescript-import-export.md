...
**module.exports** 와 **export default** 가 같다고 생각했던 사람들에게는 세번째 값이 **undefined** 가 나온 것에 대해 이상하게 생각할 수 있다. 여태껏 **Typesciprt** 가 transpile 되면서 **import** 와 **export** 를 사용하는데 있어 **default** 프로퍼티를 자동으로 처리해 주었기 때문에 기존에 사용했던 CommonJS 환경과 비슷하게 보였었던 것 뿐이었다.
...
**exports** 변수에 프로퍼티를 추가하는 형태로 transpile 되었다. **export default** 가 **module.exports** 가 아닌 <u>exports 변수에 default 프로퍼티로 추가되었다는 것</u>을 유의해야 한다.
...
**import** 를 통해 모듈을 불러와 console 에서 사용할 때에도 **default 프로퍼티**를 추가하고 transpile 되었다. 아마도 ES6 에서는 모듈을 내보낼 때 <u>default 형태</u>와 <u>property 형태</u>로 동시에 내보낼 수 있기 때문에 이렇게 처리하지 않았을까 생각해 본다. 따라서 console 에 출력된 세번째 값이 undefined 가 나온 이유는 단순히 **default 프로퍼티**가 없었기 때문이었다.  
...
<u>property 형태</u>로 내보낸 모듈을 불러오는 방식으로 선언할 경우 모듈 자체가 **import** 된다. 다시 말하면 transpile 되면서 **default 프로퍼티**를 추가하지 않고 기존의 CommonJS 방식으로 모듈을 사용할 수 있게 된다는 말이다. 위에 결과를 보면 알 수 있듯이 thirdModule 이 transpile 되면서 **default 프로퍼티**를 추가하지 않았다. console 에는 객체 정보가 그대로 출력되었다.
...
### 2. ES5(transpile) / presets: es2015
...
위에 있는 **Babel** 예제는 파일명과 변수명만 바뀌었을 뿐 **Typescript** 예제와 동일하다. 하지만 신기하게도 **Babel** 의 transpile 는 세번째 값이 재대로 출력되었다. **Typescript** 에서 **default 프로퍼티**를 처리하는 방식과 **Babel** 에서 **default 프로퍼티**를 처리하는 방식이 특별히 다르지 않은 것 같은데 어떤 이유로 이런 결과가 나올까?
...
**Babel** 로 transpile 된 js 들을 살펴보면 **Typescript** 의 transpile 에서는 없는 특별한 부분이 있다. ES6 기반의 모듈은 **__esModule 프로퍼티**를 추가로 **export** 하고 CommonJS 기반의 모듈은 그렇지 않다. 그럼 index.js 에서는 이 프로퍼티 값으로 모듈을 구분하여 추가로 **default 프로퍼티** 작업을 하게 되고 결과적으로 CommonJS 기반의 모듈은 **default 프로퍼티 값**으로 귀속된 새로운 객체로 반환된다. 따라서 CommonJS 기반의 모듈을 기존 방식으로 **import** 해도 **default 프로퍼티** 접근이 가능하기 때문에 정상적으로 출력되는 것이다.

### 3. Babel VS Typescript

**NodeJS** 에서 채택한 CommonJS 모듈 시스템에 익숙해져서 **Babel** 의 transpile 스펙이 맞는 것 같지만 이는 **Babel** 에서 편의상 제공하는 스펙이고 **Typescript** 의 transpile 이 근본적인 스펙인 것 같다. **Typescript** 와 **Babel** 은 태생이 다르기 때문에 비교하는 것은 옳지 않고 그냥 개발환경에 따라 왜 이렇게 사용하는지 어떤 차이가 있는지 이해하면서 사용하는 것이 옳다고 하겠다.
